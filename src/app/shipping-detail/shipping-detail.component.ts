...
export class ShippingDetailComponent implements OnInit,ComponentCanDeactivate {
  ...
  checkout(form:NgForm){//formu almamız için NgForm tipimde olmalı.
   if(form.invalid){
     return;
   }
    this.cartService.clear();
    this.notificationsService.info("Başarılı","Alışverişiniz Tamamlanmıştır.");
    this.router.navigate(["products"]);// bu da .ts ekranında router yapma
